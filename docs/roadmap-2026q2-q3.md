...
## 8. Retired Track — MCP Adapter

### 定位

2026-04-29 复核后退役。项目初心是建筑师在终端用 RevitCli 处理繁琐 Revit 工作,不是做 MCP / agent 接入层。

### 范围

- 不再新增 MCP 工具、资源、初始化命令或客户端文档。
- 已打开的 MCP PR 关闭或废弃,不继续合并。
- 主分支旧 `mcp serve` 代码只作为兼容遗留处理。2026-04-30 决定:
  隐藏并标记弃用,不出现在 help / completions / 新文档中;未来如移除,
  需按 breaking-change 处理。

### 替代方向

- 把开发资源投向 terminal-first 痛点:inspect/discover、安全批量 plan、交付 workflow、标准包、journal 浏览与报告。
...
- **AI 自动写 profile / 自然语言查询** — 不作为 RevitCli 主线;CLI 只保证结构化、可审计的终端能力
