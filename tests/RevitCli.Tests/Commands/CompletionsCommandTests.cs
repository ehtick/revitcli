...
public class CompletionsCommandTests : IDisposable
{
    ...
    public async Task Bash_CompletionsIncludeCommandAndValueSuggestions()
    {
        ...
        Assert.Contains("verify pilot --root --output --tag --strict --pilot-id --path --force --yes", script);
        Assert.Contains("compgen -W \"scaffold validate register status\" -- \"$cur\"", script);
    ...
    public async Task Zsh_CompletionsIncludeDoctorBatchAndConfigKeys()
    {
        ...
        Assert.Contains("_values 'subcommand' scaffold validate register status", script);
    ...
    public async Task PowerShell_CompletionsIncludeTopLevelCommandsAndConfigSuggestions()
    {
        ...
        Assert.Contains("'release' = @('verify', 'pilot', '--root', '--output', '--tag', '--strict', '--pilot-id', '--path', '--force', '--yes')", script);
        Assert.Contains("$releaseOutputFormats = @('table', 'json', 'markdown')", script);
        Assert.Contains("$releasePilotSubcommands = @('scaffold', 'validate', 'register', 'status')", script);
